...
const generate = () => {
  ...
  if (exclude.checked) { 
    ...
    if (exclusions.trim() == "" || bannedList.length === 0) { 
      ...
      results.innerHTML = `Please fill the values that must be excluded.`
      return
    }
  }
  ...
  if (Number.isNaN(minValue) || Number.isNaN(maxValue)) {
    results.className = "error"
    results.innerHTML = `Min and Max required.`
    return   
  }
  if (maxValue <= minValue) {
    results.className = "error"
    results.innerHTML = `Interval between Min and Max required.`
    return
  }
  if (norep.checked && qttValue > (maxValue - minValue - bannedList.length + 1)) {
    console.log('qtt',  maxValue - minValue - bannedList.length + 1)
    results.className = "error"
    results.innerHTML = `Quantity is bigger than interval. Please uncheck 'no repetition'.`
    return
  }
  ...
  if (exclude.checked) {
    ...
    if (list.length === 0) {
      results.className = "error"
      results.innerHTML = `All possible numbers were desconsidered.`
      return
    }
  }
