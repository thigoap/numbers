...
const startStop = () => {
  if (!running) { 
    ...
    if (Number.isNaN(hrsValue) && Number.isNaN(minValue) && Number.isNaN(secValue) ) {
      msg.className = "error"
      msg.innerHTML = `At least 1 value is necessary.`
      return      
    }
    ...
    if (hrsValue === 0 && minValue === 0 && secValue === 0) {
      msg.className = "error"
      msg.innerHTML = `Please define the time.`
      return
    }
